...
def chunk_dataset_function(examples, chunk_size, tokenizer):                                   
    ...
    for ex in zip(*values):                                                         
        ex = dict(zip(keys, ex))                                                    
        i = 0
        while i < len(ex[input_ids_str]):
            # Find the end of the current chunk
            end = min(i + chunk_size, len(ex[input_ids_str]))
            
            # Adjust the end to include complete <s> ... </s> sequence
            while end < len(ex[input_ids_str]) and ex[input_ids_str][end-1] != tokenizer.convert_tokens_to_ids('</s>'):
                end += 1

            # Extract the chunk
            chunked_input_ids_st = ex[input_ids_str][i:end]
            chunked_attention_mask = ex[attention_mask_str][i:end]

            # Add the chunk to the chunked data
            if sum(chunked_attention_mask) >= 10:  # Only add if the chunk is significant
            ...
            i = end  # Move to the start of the next chunk
...
def preprocess_function(
    ...
    if not isinstance(examples[input_column][0], str):
        ...
        model_inputs = _preprocess_raw_inputs(assign_zero_to_too_long_val_examples,
                examples,
                input_column,
                input_prefix_column,
                max_source_length,
...
def _preprocess_raw_inputs(assign_zero_to_too_long_val_examples, examples, input_column, input_prefix_column,
    ...
    if input_prefix_column in examples.keys():
        ...
        if pad_prefix:
            model_prefix_inputs = tokenizer(input_prefixes, max_length=max_prefix_length, padding='max_length', truncation=True, add_special_tokens=False)
        else:
            # for led, we do not pad the prefix
            model_prefix_inputs = tokenizer(input_prefixes, max_length=max_source_length, padding='do_not_pad', truncation=True, add_special_tokens=False)

    if trim_very_long_strings:
        inputs = [inp[: max_source_length * 7] for inp in inputs]
    model_inputs = tokenizer(inputs, max_length=max_source_length, padding=padding, truncation=True, add_special_tokens=False)

    if max_source_length is not None and assign_zero_to_too_long_val_examples:
        model_inputs_untrimmed = tokenizer(inputs, add_special_tokens=False)
    ...
    if targets is not None:
        if not isinstance(targets[0], str):
            ...
            with tokenizer.as_target_tokenizer():
                labels = tokenizer(targets, max_length=max_target_length, padding=padding, truncation=True, add_special_tokens=False)
